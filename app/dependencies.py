...
def isUrlSafeBase64(securityToken: str) -> dict:
    ...
    returnValue = {}
    try:
        unusedValue = base64.urlsafe_b64decode(securityToken)
        returnValue['status'] = True
        return (returnValue)
    except:
...
def checkApiToken(apiToken: str, salt: str, password: str, srcIP: str) -> dict:
    """ Decrypts security token and validates vairous data including the src IP and returns object data from site config including url and MISP api token.
...
def orgConfigExtraction(decryptedConfigToken: str) -> dict:
    ...
    :param decryptedConfigToken: This is the base64 urlsafe decoded string, in a semicolon-seperated format, composed of 5 elements.
...
def validateStringBool(plainText: str) -> bool:
    ...
    if (len(configData) == 5): # There has to be excactly 5 parameters
...
def memcacheAddData(dataKey: str, dataValue: str, dataExpire: int) -> bool:
    ...
    """        
    try:
        mc = bmemcached.Client(configCore['memcached_host'] + ':' + str(configCore['memcached_port']), configCore['memcached_user'], configCore['memcached_pass'])
